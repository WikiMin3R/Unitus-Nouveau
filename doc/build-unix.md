UNIX BUILD NOTES
====================
Some notes on how to build Unitus in Unix.
...
This will build unitus-qt as well if the dependencies are met.
...
memory available when compiling Unitus. On systems with less, gcc can be
...
See the section "Disable-wallet mode" to build Unitus without wallet.
...
If you want to build Unitus-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a unitus-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip unitusd" to strip the debug
...
# Configure Unitus to use our own-built instance of BDB
cd $BITCOIN_ROOT
./autogen.sh
...
As mentioned above, when maintaining portability of the wallet between the standard Unitus distributions and independently built
