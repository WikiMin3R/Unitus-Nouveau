                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
    ...
    //Unitus: in v2 transactions use GetAdjustedTime() as nTimeTx
...
CAmount GetProofOfWorkSubsidy(int nHeight)
{
    ...
    if (nHeight<101) // POW Premine phase for coin swap from former chain
    {
        return CAmount(1000000 * COIN);
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    //Unitus also requires DER encoding of pubkeys
    flags |= SCRIPT_VERIFY_DERKEY;
    //Unitus also requires low S in sigs
